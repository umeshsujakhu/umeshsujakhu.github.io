...
function executeCommand(command) {
  ...
  if (command === "help") {
    ...
    if (
      ["about", "experience", "skills", "projects", "contact"].includes(section)
    ) {
      scrollToSection(section);
      addTerminalResponse(`Navigating to ${section} section...`);
    } else {
      addTerminalResponse(
        `Unknown section: ${section}. Available sections: about, experience, skills, projects, contact`
      );
    }
  } else {
    addTerminalResponse(
...
function clearTerminal() {
  ...
  const lines = document.querySelectorAll(
    ".terminal-content .line:not(.input-line)"
  );
  lines.forEach((line) => {
...
function addTerminalResponse(content) {
  ...
  document
    ...
    .insertBefore(responseDiv, inputLine);
}
...
function initTypeableTerminal() {
  ...
  const blinkingCursorLine = document.querySelector(
    ".terminal-content .line:last-child"
  );
    ...
    if (e.key === "Enter") {
      ...
      if (input) {
        ...
        if (matches.length === 1) {
          ...
          addTerminalResponse(
            `<div class="completion-suggestions">${matches.join(", ")}</div>`
          );
        }
      }
    } else {
  ...
  setTimeout(() => {
    addTerminalResponse(
      'Welcome to Umesh\'s portfolio terminal. Type <span class="command-highlight">help</span> for available commands.'
    );
  }, 4000);
}
...
function createMatrixRain() {
  ...
  // Use a wider character set with programming symbols
  const characters =
    "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789$><=!?{}[]()+-*/%&|^~#@;:,.\\'\"`";

  // Create more columns for better density
  const columns = Math.floor(window.innerWidth / 30);

  // Increase drop density
  const dropDensity = 0.8; // Create 80% of possible columns
  ...
  for (let i = 0; i < columns; i++) {
    ...
    drop.style.left = i * 30 + "px"; // Closer spacing between columns

    // Faster animation duration (3-7 seconds)
    drop.style.animationDuration = Math.random() * 4 + 3 + "s";

    // Shorter delays between drops (0-3 seconds)
    drop.style.animationDelay = Math.random() * 3 + "s";

    // Higher opacity for better visibility
    drop.style.opacity = "0.5";

    drop.textContent = characters.charAt(
  ...
  style.textContent += `
        ...
        .matrix-drop {
            position: absolute;
            top: -20px;
            color: rgba(0, 255, 65, 0.7); /* Brighter green with higher opacity */
            font-family: var(--font-code), 'Courier New', monospace;
            font-size: 14px; /* Larger font size */
            text-shadow: 0 0 2px rgba(0, 255, 65, 0.5); /* Stronger glow */
            animation-name: matrix-fall;
            animation-timing-function: linear; /* Linear for consistent speed */
            animation-iteration-count: infinite;
            filter: blur(0.3px); /* Less blur */
        }
  ...
  contactForm.addEventListener("submit", function (e) {
    ...
    setTimeout(() => {
      ...
      submitButton.innerHTML =
        '<span class="button-text">Message Sent!</span><span class="button-icon"><i class="fas fa-check"></i></span>';
